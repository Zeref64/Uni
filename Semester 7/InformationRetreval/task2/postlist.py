...
def preprocessing(final_string):
    ...
    # Remove punctuations.
    table = str.maketrans('', '', '\t')
    final_string = [word.translate(table) for word in final_string]
    punctuations = (string.punctuation).replace("'", "")
    trans_table = str.maketrans('', '', punctuations)
    stripped_words = [word.translate(trans_table) for word in final_string]
...
def mostCommon3(text):
    ...
    wordsList_Tuple = fdistText.most_common(3)

    wordList_final = []
    for i in range(len(wordsList_Tuple)):
        wordList_final.append(str(wordsList_Tuple[i][0]))
        print(str(wordsList_Tuple[i][0]))
    print(wordList_final)
...
# Initialize the dictionary.
    ...
    text7_3_MostCommon = mostCommon3(text7)
    final_token_list = cleanText(text7_3_MostCommon, "english")
